    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim, HS_MSA(cfg, dim=dim, window_size=window_size, dim_head=dim_head, heads=heads, only_local_branch=(heads==1))),
...
class HST(nn.Module):
    def __init__(self, cfg, in_dim=28, out_dim=28, dim=28, num_blocks=[1,1,1]):
        ...
        for i in range(self.scales-1):
            self.encoder_layers.append(nn.ModuleList([
                HSAB(cfg, dim=dim_scale, window_size=cfg.MODEL.DAUHST.WINDOW_SIZE, num_blocks=num_blocks[i], dim_head=dim, heads=dim_scale // dim),
                nn.Conv2d(dim_scale, dim_scale * 2, 4, 2, 1, bias=False),
            ]))
            dim_scale *= 2

        # Bottleneck
        self.bottleneck = HSAB(cfg, dim=dim_scale, window_size=cfg.MODEL.DAUHST.WINDOW_SIZE, dim_head=dim, heads=dim_scale // dim, num_blocks=num_blocks[-1])
        ...
        for i in range(self.scales-1):
            self.decoder_layers.append(nn.ModuleList([
                ...
                HSAB(cfg, dim=dim_scale // 2, window_size=cfg.MODEL.DAUHST.WINDOW_SIZE, num_blocks=num_blocks[self.scales - 2 - i], dim_head=dim,
...
class DAUHST(BaseModel):

    def __init__(self, cfg):
        super(DAUHST, self).__init__(cfg)
        num_iterations = self.cfg.MODEL.DAUHST.STAGES
        self.para_estimator = HyPaNet(in_nc=self.cfg.DATASETS.WAVE_LENS, out_nc=num_iterations*2)
        self.fution = nn.Conv2d(self.cfg.DATASETS.WAVE_LENS * 2, self.cfg.DATASETS.WAVE_LENS, 1, padding=0, bias=True)
        self.num_iterations = num_iterations
        self.denoisers = nn.ModuleList([])
        for _ in range(num_iterations):
            self.denoisers.append(
                HST(cfg, in_dim=self.cfg.DATASETS.WAVE_LENS+1+1, out_dim=self.cfg.DATASETS.WAVE_LENS, dim=self.cfg.DATASETS.WAVE_LENS, num_blocks=[1,1,1]),
            )
    ...
    def initial(self, y, Phi):
        ...
        """
        nC, step = self.cfg.DATASETS.WAVE_LENS, self.cfg.DATASETS.STEP
        y = y / nC * 2
        bs,row,col = y.shape
    ...
    def forward_train(self, data):
        ...
        for i in range(self.num_iterations):
            ...
            x = shift_back_3d(x, step=self.cfg.DATASETS.STEP)[:, :, :, :W]
            beta_repeat = beta.repeat(1,1,x.shape[2], x.shape[3])
            z = self.denoisers[i](torch.cat([x, yp.unsqueeze(1), beta_repeat],dim=1))
...
class DAUHST_Profiling(BaseModel):

    def __init__(self, cfg):
        super(DAUHST_Profiling, self).__init__(cfg)
        num_iterations = self.cfg.MODEL.DAUHST.STAGES
        self.para_estimator = HyPaNet(in_nc=self.cfg.DATASETS.WAVE_LENS, out_nc=num_iterations*2)
        self.fution = nn.Conv2d(self.cfg.DATASETS.WAVE_LENS * 2, self.cfg.DATASETS.WAVE_LENS, 1, padding=0, bias=True)
        self.num_iterations = num_iterations
