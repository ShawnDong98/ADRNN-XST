...
def FFN_FN(
    ...
    if ffn_name == "Gated_Dconv_FeedForward":
        return Gated_Dconv_FeedForward(
                dim, 
                ffn_expansion_factor=cfg.MODEL.DERNN_LNLT.FFN_EXPAND, 
            )
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim, LocalMSA(
                    ...
                    layernorm_type = layernorm_type) if self.cfg.MODEL.DERNN_LNLT.LOCAL else nn.Identity(),
                PreNorm(dim, NonLocalMSA(
                        dim = dim, 
                        num_heads = num_heads,
                        window_num = window_num,
                    ),
                    layernorm_type = layernorm_type) if self.cfg.MODEL.DERNN_LNLT.NON_LOCAL else nn.Identity(),
                PreNorm(dim, FFN_FN(
                    cfg,
                    ffn_name = cfg.MODEL.DERNN_LNLT.FFN_NAME,
                    dim = dim
                ),
...
class LNLT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.DATASETS.WITH_PAN:
            self.embedding = nn.Conv2d(cfg.MODEL.DERNN_LNLT.IN_DIM + 1, cfg.MODEL.DERNN_LNLT.DIM, kernel_size=3, stride=1, padding=1, bias=False)
        else:
            self.embedding = nn.Conv2d(cfg.MODEL.DERNN_LNLT.IN_DIM, cfg.MODEL.DERNN_LNLT.DIM, kernel_size=3, stride=1, padding=1, bias=False)


        self.Encoder = nn.ModuleList([
            LocalNonLocalBlock(
                cfg = cfg, 
                dim = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 0, 
                num_heads = 2 ** 0, 
                window_size = cfg.MODEL.DERNN_LNLT.WINDOW_SIZE,
                window_num = cfg.MODEL.DERNN_LNLT.WINDOW_NUM,
                layernorm_type = cfg.MODEL.DERNN_LNLT.LAYERNORM_TYPE,
                num_blocks = cfg.MODEL.DERNN_LNLT.NUM_BLOCKS[0],
            ),
            LocalNonLocalBlock(
                cfg = cfg, 
                dim = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 1, 
                num_heads = 2 ** 1, 
                window_size = cfg.MODEL.DERNN_LNLT.WINDOW_SIZE,
                window_num = cfg.MODEL.DERNN_LNLT.WINDOW_NUM,
                layernorm_type = cfg.MODEL.DERNN_LNLT.LAYERNORM_TYPE,
                num_blocks = cfg.MODEL.DERNN_LNLT.NUM_BLOCKS[1],
            ),
        ])

        self.BottleNeck = LocalNonLocalBlock(
                cfg = cfg, 
                dim = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 2, 
                num_heads = 2 ** 2, 
                window_size = cfg.MODEL.DERNN_LNLT.WINDOW_SIZE,
                window_num = cfg.MODEL.DERNN_LNLT.WINDOW_NUM,
                layernorm_type = cfg.MODEL.DERNN_LNLT.LAYERNORM_TYPE,
                num_blocks = cfg.MODEL.DERNN_LNLT.NUM_BLOCKS[2],
            )
        ...
        self.Decoder = nn.ModuleList([
            LocalNonLocalBlock(
                ...
                window_num = cfg.MODEL.DERNN_LNLT.WINDOW_NUM,
                layernorm_type = cfg.MODEL.DERNN_LNLT.LAYERNORM_TYPE,
                num_blocks = cfg.MODEL.DERNN_LNLT.NUM_BLOCKS[3],
            ),
            LocalNonLocalBlock(
                cfg = cfg, 
                dim = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 0, 
                ...
                window_num = cfg.MODEL.DERNN_LNLT.WINDOW_NUM,
                layernorm_type = cfg.MODEL.DERNN_LNLT.LAYERNORM_TYPE,
                num_blocks = cfg.MODEL.DERNN_LNLT.NUM_BLOCKS[4],
            )
        ])

        self.Downs = nn.ModuleList([
            DownSample(cfg.MODEL.DERNN_LNLT.DIM * 2 ** 0),
            DownSample(cfg.MODEL.DERNN_LNLT.DIM * 2 ** 1)
        ])

        self.Ups = nn.ModuleList([
            UpSample(cfg.MODEL.DERNN_LNLT.DIM * 2 ** 2),
            UpSample(cfg.MODEL.DERNN_LNLT.DIM * 2 ** 1)
        ])

        self.fusions = nn.ModuleList([
            nn.Conv2d(
                in_channels = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 2,
                out_channels = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 1,
                kernel_size = 1,
                stride = 1,
                padding = 0,
                bias = False
            ...
            nn.Conv2d(
                in_channels = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 1,
                out_channels = cfg.MODEL.DERNN_LNLT.DIM * 2 ** 0,
                kernel_size = 1,
                stride = 1,
                padding = 0,
                bias = False
        ...
        self.mapping = nn.Conv2d(cfg.MODEL.DERNN_LNLT.DIM, cfg.MODEL.DERNN_LNLT.OUT_DIM, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.cfg.MODEL.DERNN_LNLT.WITH_NOISE_LEVEL:
...
class DERNN_LNLT(BaseModel):
    ...
    def __init__(self, cfg):
        ...
        self.DP = nn.ModuleList([
           DegradationEstimation(cfg) for _ in range(cfg.MODEL.DERNN_LNLT.STAGES)
        ]) if not cfg.MODEL.DERNN_LNLT.SHARE_PARAMS else DegradationEstimation(cfg)
        self.PP = nn.ModuleList([
            LNLT(cfg) for _ in range(cfg.MODEL.DERNN_LNLT.STAGES)
        ]) if not cfg.MODEL.DERNN_LNLT.SHARE_PARAMS else LNLT(cfg)
    ...
    def forward_train(self, data):
        ...
        for i in range(self.cfg.MODEL.DERNN_LNLT.STAGES):
            Phi, mu, noise_level = self.DP[i](z, phi) if not self.cfg.MODEL.DERNN_LNLT.SHARE_PARAMS else self.DP(z, phi)

            if not self.cfg.MODEL.DERNN_LNLT.WITH_DL:
                Phi = phi
            if not self.cfg.MODEL.DERNN_LNLT.WITH_MU:
            ...
            x = shift_back_3d(x, self.cfg.DATASETS.STEP)[:, :, :, :W_]
            noise_level_repeat = noise_level.repeat(1,1,x.shape[2], x.shape[3])
            if self.cfg.DATASETS.WITH_PAN:
                x = torch.cat([x, yp.unsqueeze(1)], dim=1)
            if not self.cfg.MODEL.DERNN_LNLT.WITH_NOISE_LEVEL:
                z = self.PP[i](x) if not self.cfg.MODEL.DERNN_LNLT.SHARE_PARAMS else self.PP(x)
            else:
                z = self.PP[i](torch.cat([noise_level_repeat, x],dim=1)) if not self.cfg.MODEL.DERNN_LNLT.SHARE_PARAMS else self.PP(torch.cat([noise_level_repeat, x],dim=1))
            z = shift_3d(z, self.cfg.DATASETS.STEP)


        z = shift_back_3d(z, self.cfg.DATASETS.STEP)[:, :, :, :W_]
...
class DERNN_LNLT_Profiling(BaseModel):
    """
    The DERNN-LNLT unfolds the HQS algorithm within the MAP framework and transfors the DUN into an RNN by sharing parameters across stages.  
