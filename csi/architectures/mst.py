...
def shift_back(inputs, down_sample, step=2, wave_lens=28):          # input [bs,28,256,310]  output [bs, 28, 256, 256]
    [bs, nC, row, col] = inputs.shape
    step = float(step)/float(down_sample*down_sample)
    out_col = math.ceil(col - step * down_sample * (wave_lens - 1))
    for i in range(nC):
        inputs[:,i,:,:out_col] = \
...
class MaskGuidedMechanism(nn.Module):
    ...
    def forward(self, mask_shift):
        ...
        mask_emb = shift_back(mask_shift, self.down_sample, self.cfg.DATASETS.STEP, self.cfg.DATASETS.WAVE_LENS)
        return mask_emb
...
class MS_MSA(nn.Module):
    def __init__(
        ...
        self.mm = MaskGuidedMechanism(cfg, dim, down_sample=heads)
        self.dim = dim
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                MS_MSA(cfg, dim=dim, dim_head=dim_head, heads=heads),
...
class MST(BaseModel):
    def __init__(self, cfg):
        super(MST, self).__init__(cfg)
        self.dim = self.cfg.MODEL.MST.DIM
        self.stage = self.cfg.MODEL.MST.STAGE

        dim = self.cfg.MODEL.MST.DIM
        stage = self.cfg.MODEL.MST.STAGE
        num_blocks = self.cfg.MODEL.MST.NUM_BLOCKS

        # Input projection
        if self.cfg.DATASETS.WITH_PAN:
            self.embedding = nn.Conv2d(self.cfg.DATASETS.WAVE_LENS+1, self.dim, 3, 1, 1, bias=False)
        else:
            self.embedding = nn.Conv2d(self.cfg.DATASETS.WAVE_LENS, self.dim, 3, 1, 1, bias=False)
        ...
        # Output projection
        self.mapping = nn.Conv2d(self.dim, self.cfg.DATASETS.WAVE_LENS, 3, 1, 1, bias=False)
    ...
    def forward_train(self, data):
        if self.cfg.DATASETS.WITH_PAN:
            if self.cfg.EXPERIMENTAL_TYPE == "real":
                B, nC, H, W = data['MeasH'].shape
                data['MeasP'] = data['MeasP'] * nC / 1.2
            x = torch.cat((data['MeasH'], data['MeasP'].unsqueeze(1)), dim=1)
        else:
            x = data['MeasH']
        mask = data['mask']
        ...
        pad_h = xh - mh
        pad_w = xw + self.cfg.DATASETS.STEP * (self.cfg.DATASETS.WAVE_LENS - 1)  - mw
        ...
        # Mapping
        out = self.mapping(fea) + x[:, :self.cfg.DATASETS.WAVE_LENS, :, :]
...
class MST_Profiling(BaseModel):
    def __init__(self, cfg):
        super(MST_Profiling, self).__init__(cfg)
        self.dim = self.cfg.MODEL.MST.DIM
        self.stage = self.cfg.MODEL.MST.STAGE
