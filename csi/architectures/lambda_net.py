...
class Lambda_Net(BaseModel):
    def __init__(self, cfg):
        super(Lambda_Net, self).__init__(cfg)
        in_ch = self.cfg.MODEL.LAMBDA_NET.IN_CH
        out_ch = self.cfg.MODEL.LAMBDA_NET.OUT_CH

        if self.cfg.DATASETS.WITH_PAN:
            self.conv_in = nn.Conv2d(in_ch+1+1, out_ch, 3, padding=1)
        else:
            self.conv_in = nn.Conv2d(in_ch+1, out_ch, 3, padding=1)

        # encoder
        self.conv_down1 = triple_conv(out_ch, 32)
    ...
    def forward_train(self, data):
        ...
        if self.cfg.DATASETS.WITH_PAN:
            if self.cfg.EXPERIMENTAL_TYPE == "real":
                B, nC, _, _ = phi_c.shape
                yp = yp * nC / 1.2
            B, HC, WC = x.shape
            B, HP, WP = yp.shape
            yp = F.pad(yp, [0, WC - WP, 0, 0], mode='constant')
            x = self.conv_in(torch.cat([x.unsqueeze(1), yp.unsqueeze(1), input_mask], dim=1))
        else:
...
class Lambda_Net_Profiling(BaseModel):
    def __init__(self, cfg):
        super(Lambda_Net_Profiling, self).__init__(cfg)
        in_ch = self.cfg.MODEL.LAMBDA_NET.IN_CH
        out_ch = self.cfg.MODEL.LAMBDA_NET.OUT_CH

        if self.cfg.DATASETS.WITH_PAN:
            self.conv_in = nn.Conv2d(in_ch+1+1, 28, 3, padding=1)
        else:
            self.conv_in = nn.Conv2d(in_ch+1, 28, 3, padding=1)
